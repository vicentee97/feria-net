...
function hasSourceCode(result) {
  return Boolean(result && result.sourceCode);
}

async function resolveResultsWithFallback(components, apiResults) {
  const resultsByComponent = new Map(
    (apiResults || []).map((result) => [result.component, {...result, source: "api"}]),
  );

  const resolved = [];
  for (const component of components) {
    const apiResult = resultsByComponent.get(component);

    if (hasSourceCode(apiResult)) {
      resolved.push(apiResult);
      continue;
    }

    const fallbackResult = await fetchGithubFallback(component);
    resolved.push(hasSourceCode(fallbackResult) ? fallbackResult : (apiResult || fallbackResult));
  }
...
async function main() {
  ...
  if (data && data.results) {
    const resolvedResults = await resolveResultsWithFallback(components, data.results);

    // Output results
    if (resolvedResults.length === 1) {
      const result = resolvedResults[0];
      ...
      console.log(JSON.stringify({results: resolvedResults}, null, 2));
    }
