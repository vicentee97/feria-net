...
function hasDocsContent(result) {
  return Boolean(result && result.content);
}
...
async function resolveResultsWithFallback(components, apiResults) {
  ...
  for (const component of components) {
    ...
    if (hasDocsContent(apiResult)) {
      resolved.push(apiResult);
      continue;
    }

    const fallbackResult = await fetchFallback(component);
    resolved.push(hasDocsContent(fallbackResult) ? fallbackResult : (apiResult || fallbackResult));
  }
