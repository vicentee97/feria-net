...
function hasStylesCode(result) {
  return Boolean(result && result.stylesCode);
}
...
async function resolveResultsWithFallback(components, apiResults) {
  ...
  for (const component of components) {
    ...
    if (hasStylesCode(apiResult)) {
      resolved.push(apiResult);
      continue;
    }

    const fallbackResult = await fetchGithubFallback(component);
    resolved.push(hasStylesCode(fallbackResult) ? fallbackResult : (apiResult || fallbackResult));
  }
